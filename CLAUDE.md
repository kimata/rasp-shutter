...
スケジュール更新は `tests/helpers/api_utils.py` の `ScheduleAPI.update()` を使用する。この関数は保存（`queue.put`）後、スケジューラスレッドがジョブ登録を終える（スケジュール適用世代番号が進む）まで待機してから返る。**適用を確認せずに時間を移動すると、ジョブ登録が予定時刻を過ぎてから行われ、`schedule` ライブラリが `next_run` を翌日に計算してジョブが発火しなくなる**（過去に flaky テストの原因になった）。
...
スケジューラはループごとにシーケンス番号を、スケジュール適用ごとに適用世代番号をインクリメントする。テストは以下の API でスケジューラの状態を確認できる：
...
- `GET /api/test/scheduler/schedule_generation` - 現在のスケジュール適用世代番号を取得
- `POST /api/test/scheduler/wait_schedule_applied?generation=N&timeout=T` - 適用世代番号が N を超えるまで待機

**注意**: ループシーケンスは「ループが回った」ことしか保証しない。スケジュール保存後に時刻を進める場合は、必ず適用世代番号で「保存したスケジュールがジョブ登録された」ことを確認すること（E2E では `save_schedule_and_wait()` を使用）。`queue.put()` の可視化遅延により、ループが複数周してもキューが未処理のことがある。
