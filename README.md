...
コード構造の詳細（ブループリント構成、スケジューラスレッド、footprint による状態管理、メトリクスの 3 層構成など）は [アーキテクチャドキュメント](doc/architecture.md) を参照してください。
...
- **見合わせイベント** - センサー条件で見送られたスケジュール/自動制御の発生時刻・理由・閾値（解消時刻・所要時間を含む）
- **センサーサンプル** - スケジューラループから 1 分間隔で記録する照度・日射・太陽高度（時刻別の傾向分析用）

### 主な機能

- **📊 操作統計グラフ** - 基本統計、時刻分析、時系列データの可視化
- **⏸ 見合わせ分析** - 直近30日の見合わせ件数、理由別/方向別の内訳、解消ラグ、詳細イベント表
- **📐 閾値マージン分析** - 操作時のセンサー値が現在の閾値からどれだけ離れていたかを可視化（閾値チューニング向け）
- **🌅 センサー日内推移** - 1分サンプルを時刻別に散布表示し、開け/閉め閾値を重ね描き
...
- [アーキテクチャ](doc/architecture.md) - コード構造の解説（SVG 図解付き）。全体構成、Flask ブループリント、スケジューラスレッド、自動制御の判定フロー、footprint による状態管理、メトリクスの 3 層構成、フロントエンド構成など
