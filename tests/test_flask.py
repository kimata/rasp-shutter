...
def time_str(time, offset_hour=OFF):
    return (time + datetime.timedelta(hours=offset_hour)).strftime("%H:%M")
...
def gen_schedule_data():
    ...
    return {
        ...
        "close": schedule_data | {"time": "18:00", "solar_rad": 80, "lux": 1200},
    }
...
def test_shutter_ctrl_inconsistent_read(client):
    ...
    rasp_shutter_control.stat_exec_file("open", 0).touch()
    time.sleep(1)
    rasp_shutter_control.stat_exec_file("close", 0).touch()
    ...
    rasp_shutter_control.stat_exec_file("open", 0).unlink(missing_ok=True)
    rasp_shutter_control.stat_exec_file("close", 0).unlink(missing_ok=True)
...
def test_valve_ctrl_manual_single(client):
    response = client.get(
        "/rasp-shutter/api/shutter_ctrl",
        query_string={
            "cmd": 1,
...
def test_schedule_ctrl_auto_close_dup(client, mocker, freezer):
    ...
    mocker.patch("scheduler.elapsed_time", return_value=0)
    sensor_data_mock.return_value = SENSOR_DATA_BRIGHT
...
def test_schedule_ctrl_open_dup(client, mocker, freezer):
    ...
    mocker.patch("rasp_shutter_control.elapsed_time", return_value=0)
...
def test_schedule_ctrl_control_fail_1(client, mocker, freezer):
    ...
    schedule_data = gen_schedule_data()
    schedule_data["close"]["time"] = time_str(time_evening(2))
    schedule_data["open"]["is_active"] = False
    response = client.get(
        "/rasp-shutter/api/schedule_ctrl",
        query_string={"cmd": "set", "data": json.dumps(schedule_data)},
