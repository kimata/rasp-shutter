...
def shutter_auto_control_impl(url):
    try:
...
def shutter_auto_control(url):
    logging.info("Starts automatic control of the shutter")

    for i in range(RETRY_COUNT):
        if shutter_auto_control_impl(url):
            return True
...
def schedule_validate(schedule_data):
    ...
    for name, entry in schedule_data.items():
        for key in ["is_active", "time", "wday"]:
            if key not in entry:
                return False
...
def set_schedule(schedule_data):
    ...
    for name, entry in schedule_data.items():
        ...
        def func():
            shutter_auto_control(entry["endpoint"])
